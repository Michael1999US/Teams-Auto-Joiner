...
I also made a short tutorial video on how to set up the bot: https://youtu.be/YgkSOqfIjf4
...
- **greeting:**     
Send custom text into the reply box. For example: "Hi, I'm present."

- **run_at_time:**  
Time the scrip will start. Input is a string of the hour and minute in 24h format if you want it to occur immediately leave this empty. 
...
If set to a value greater than zero, the bot leaves every meeting after the specified time (in minutes). Useful if you know the length of your session if this is left a the default the bot will stay in the meeting until a new one is available.

- **leave_if_last:**
If true, leaves the meeting if you are the last person in it.

- **headless:**
If true, runs Chrome in headless mode (does not open GUI window and runs in the background).
...
- **blacklist:**
A list of Teams and their channels to ignore. Meetings in these channels will not be joined.
...
If calendar meeting title matches a regular expression, it goes to the blacklist.
Leave empty to attend all meetings.
